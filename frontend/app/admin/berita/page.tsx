...
function ImageUploadCrop({ value, onChange, onPendingChange, onRegisterUpload }: { value: string; onChange: (url: string) => void; onPendingChange?: (hasPending: boolean) => void; onRegisterUpload?: (fn: () => Promise<string | null>) => void }) {
  ...
  // Sync fileType and preview with value prop changes (e.g. when editing news)
  useEffect(() => {
    setPreview(value || "");
    if (value) {
  ...
  const calcLayout = useCallback((img: HTMLImageElement): ImgLayout => {
    ...
    if (ia > ca) { const h = CANVAS_W / ia; return { x: 0, y: (CANVAS_H - h) / 2, w: CANVAS_W, h }; }
    const w = CANVAS_H * ia; return { x: (CANVAS_W - w) / 2, y: 0, w, h: CANVAS_H };
  }, []);
  ...
  useEffect(() => { if (imgEl && imgLayout.w > 0) drawCanvas(imgEl, imgLayout, crop); }, [imgEl, imgLayout, crop, drawCanvas]);
  ...
  const onMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (dragging.type === "move") {
      setCrop({ ...crop, x: Math.max(il.x, Math.min(il.x + il.w - sc.w, sc.x + dx)), y: Math.max(il.y, Math.min(il.y + il.h - sc.h, sc.y + dy)) });
    } else {
  ...
  const handleUpload = async (): Promise<string | null> => {
    ...
    try {
      ...
      // Draw image onto offscreen canvas (transparency is preserved by default)
      ctx.drawImage(imgEl, srcX, srcY, srcW, srcH, 0, 0, srcW, srcH);
      
      // Save matching the original format to preserve transparency (PNG/WEBP/JPEG)
      ...
      const url = await api.admin.uploadImage(blob, `news_${Date.now()}.${fileExt}`);
      setPreview(url);
  ...
  const reset = () => { setImgEl(null); setImgLayout({ x:0,y:0,w:0,h:0 }); setPreview(""); setError(""); onChange(""); onPendingChange?.(false); if (fileRef.current) fileRef.current.value = ""; };
    ...
    <div className="space-y-3">
      ...
      {imgEl && (
        <div className="space-y-2">
          ...
          {/* Wrapper holds the aspect ratio so the canvas never gets squished */}
